...
object WLP4Scan {
   ...
   */
  val Keywords: Set[String] = Set("int", "wain", "new", "delete", "if", "else", "while", "return", "println", "NULL")

  /* A list of all tokens which can be generated by scanning CS241 MIPS assembly code */
  val Tokens: Set[String]      = Set("ID", "NUM", "LPAREN", "RPAREN", "LBRACE", "RBRACE", "RETURN", "IF", "ELSE",
                                    "WHILE", "PRINTLN", "WAIN", "BECOMES", "INT", "EQ", "NE", "LT", "GT", "LE", "GE",
                                    "PLUS", "MINUS", "STAR", "SLASH", "PCT", "COMMA", "SEMI", "NEW", "DELETE", "LBRACK",
                                    "AMP", "NULL", "ZERO")
  ...
  val asmDFA: DFA = {
    ...
    val dfaAccepting: Set[String] = Tokens ++ Set("WHITESPACE", "COMMENT")
    val dfaStates:    Set[String] = dfaAccepting ++ Set("ERROR", "START")
    ...
    DFA(
      ...
      transition = {
        ...
        //case ("START",'0')                                => "ZERO"
        //case ("ZERO", x)      if x.isLetterOrDigit        => "ZEROMINUS"
    ...
    def scanOne(input: List[Char], state: String = dfa.start, consumedInput: List[Char] = Nil): (List[Char], Token) =
      if (input.isEmpty || !dfa.transition.isDefinedAt((state,input.head)))
        if (dfa.accepting contains state) (input,Token(state,consumedInput.reverse.mkString))
          ...
          //println("failing on state " + state);
  ...
  def scan(input: String): Seq[Token] = {
    ...
    //println(input)
    //println("scan called")
    var tokens = simplifiedMaximalMunch(asmDFA, input.toList);
    //tokens.map(x => x.prettyPrint)

    tokens = tokens.map {
      case Token("ID", x) if Keywords contains x                                         =>
        Token(x.toUpperCase, x)
      case Token("ZERO",s)                                                               =>
        Token("INT",s)
      case t                                                                             =>
        t
    ...
    for (i <- 0 to tokens.length - 2){
      val curr = tokens.apply(i).kind; 
      if ( (curr == "NUM" ) && (tokens.apply(i).lexeme.length > 1 )) {
        if (tokens.apply(i).lexeme.charAt(0) == '0') {
          Console.err.println("ERROR: invalid number " + tokens.apply(i).lexeme);
          System.exit(1);
        }
      } 
      val next = tokens.apply(i+1).kind;
      val s1 = Set("ID", "NUM", "RETURN", "IF", "ELSE", "WHILE", 
        "PRINTLN", "WAIN", "INT", "NEW", "NULL", "DELETE");
      val s2 = Set("EQ", "NE", "LT", "LE", "GT", "GE", "BECOMES");
      if (( (s1 contains curr) && (s1 contains next )) || ( (s2 contains curr) && (s2 contains next ))) {
        Console.err.println("ERROR: whitespace needed")
        System.exit(1);
      }

    }
    tokens = tokens.filter(Tokens contains _.kind) /* Remove whitespace and comments */
   // return tokens;
    ...
    def checkRange(t: Token): Unit = {
      //t.prettyPrint()
        ...
        t match {
          case Token("NUM",x)    =>
            if (t.toLong > 2147483647 || x.toLong < -2147483648l) {
              Console.err.println("ERROR: VIBInteger out of range: ${t.lexeme}")
              System.exit(1);
            }
          case _                   => Unit
        }
  ...
  def main(args: Array[String]): Unit = {
    ...
    var tokenLines = Seq[Seq[Token]]();
    tokenLines = input.map(x => scan(x));
    //println("scaan complete")
    for (l <- tokenLines) {
      l.map(x => x.prettyPrint())
    }
    //println("input " + input);
    //var tokenLines: Seq[Seq[Token]] = io.Source.stdin.getLines.map(scan).toSeq;
    //tokenLines.map(println)
  }
}
