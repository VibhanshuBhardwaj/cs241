...
object AddNodeObj {
	def isLeftChildAvailable(n: Node) : Boolean = { //just checks if left child of n is avaailable for filling
	...
	def AddNode(curr: Node, data: Int, nChildren: Int) {
		//println("curr " + curr.Data + " " + curr.NumberOfChildren)
		if (curr.Data == -1) {
			curr.Data = data;
			curr.NumberOfChildren = nChildren;
			return;
		}
		//println("isLeftChildAvailable(curr) "+ isLeftChildAvailable(curr));
		if (isLeftChildAvailable(curr)) {
			...
			//println("appended " + data + " " + nChildren)
		}
